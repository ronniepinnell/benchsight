...
def add_tracking(supabase: Client, game_id: int) -> dict:
    ...
    try:
        ...
        if 'events' in xl.sheet_names:
            print(f"  Adding to fact_event_players...")
            ...
            for i in range(0, len(records), batch_size):
                batch = records[i:i + batch_size]
                try:
                    supabase.table('fact_event_players').insert(batch).execute()
                    results['events'] += len(batch)
                except Exception as e:
                    print(f"    Batch error: {e}")
            
            print(f"  ✓ fact_event_players: {results['events']} rows")
        
        # Shifts
        if 'shifts' in xl.sheet_names:
            print(f"  Adding to fact_shifts...")
            ...
            for i in range(0, len(records), 50):
                ...
                try:
                    supabase.table('fact_shifts').insert(batch).execute()
                    results['shifts'] += len(batch)
                except Exception as e:
                    print(f"    Batch error: {e}")
            
            print(f"  ✓ fact_shifts: {results['shifts']} rows")
