...
class FKBuilder:
    ...
    def add_shifts_fkeys(self):
        """Add FKs to fact_shift_players."""
        logger.info("Adding FKs to fact_shift_players...")
        ...
        df.to_csv(OUTPUT_DIR / 'fact_shift_players.csv', index=False)
        logger.info(f"  ✓ fact_shift_players: Added FKs")
        return df
    
    def add_events_fkeys(self):
        """Add FKs to fact_event_players."""
        logger.info("Adding FKs to fact_event_players...")
        
        df = pd.read_csv(OUTPUT_DIR / 'fact_event_players.csv', low_memory=False)
        ...
        df.to_csv(OUTPUT_DIR / 'fact_event_players.csv', index=False)
        logger.info(f"  ✓ fact_event_players: Added FKs")
        return df
    ...
    def add_chains_fkeys(self):
        """Add FKs to fact_shot_chains."""
        logger.info("Adding FKs to fact_shot_chains...")
        
        chains_file = OUTPUT_DIR / 'fact_shot_chains.csv'
        if not chains_file.exists():
            logger.warning("fact_shot_chains.csv not found")
            return
        ...
        df.to_csv(chains_file, index=False)
        logger.info(f"  ✓ fact_shot_chains: Added FKs")
