...
def combine_all_tracking():
    ...
    if all_events:
        ...
        # Save
        events_path = os.path.join(OUTPUT_DIR, 'fact_event_players.csv')
        combined_events.to_csv(events_path, index=False)
        print(f"fact_event_players.csv: {len(combined_events)} rows")
    ...
    if all_shifts:
        ...
        # Save
        shifts_path = os.path.join(OUTPUT_DIR, 'fact_shifts.csv')
        combined_shifts.to_csv(shifts_path, index=False)
        print(f"fact_shifts.csv: {len(combined_shifts)} rows")
