...
**This file is the SOURCE OF TRUTH for issue order and priority.**

| Metric | Count |
|--------|-------|
| **Documented Issues** | ~211 |
| **In GitHub** | 46 |
| **Open** | 44 |
| **Closed** | 2 |
| **Gap (not yet created)** | ~165 |
...
| Trigger | Action |
|---------|--------|
| Phase 2 ~75% complete | Create Phase 3 P0/P1 issues |
| Starting new phase | Create remaining P2/P3 for that phase |
| New requirement discovered | Add to backlog doc first, then create if current phase |
...
| Milestone | Phases | Description | GitHub Link |
|-----------|--------|-------------|-------------|
| **M1: MVP Foundation** | 1-4 | Core platform functional | [View](https://github.com/ronniepinnell/benchsight/milestone/1) |
| **M2: Tracker Modernization** | 5 | Rust/Next.js tracker | [View](https://github.com/ronniepinnell/benchsight/milestone/2) |
| **M3: ML/CV Integration** | 6 | Automated tracking | [View](https://github.com/ronniepinnell/benchsight/milestone/3) |
| **M4: Commercial Ready** | 7-8 | Multi-tenant, payments | [View](https://github.com/ronniepinnell/benchsight/milestone/4) |
| **M5: AI Coaching Platform** | 9-12 | AI coach, NL queries, coach/GM modes | [View](https://github.com/ronniepinnell/benchsight/milestone/5) |

### Milestone Progress Commands

```bash
# View milestone progress
gh api repos/ronniepinnell/benchsight/milestones --jq '.[] | "\(.title): \(.open_issues) open, \(.closed_issues) closed"'

# List issues in a milestone
gh issue list --milestone "M1: MVP Foundation"

# Move issue to milestone
gh issue edit [NUMBER] --milestone "M1: MVP Foundation"
```
...
### P0 Issues (Do First - IN ORDER)

| Order | # | Title | Depends On | Status |
|-------|---|-------|------------|--------|
| ✓ | #3 | ETL-001: Modularize base_etl.py | None | ✅ CLOSED |
| 1 | #31 | Investigate missing 7 tables (132 vs 139) | None | **DO FIRST** |
| 2 | #35 | ETL-013: Comprehensive table verification framework | #31 | Open |
| 3 | #36 | ETL-035: Unit tests for critical calculations | #35 | Open |
| 4 | #13 | ETL-011: Verify goal counting | #35 | Open |
| 5 | #37 | ETL-032: Comprehensive error handling | #36 | Open |
| 6 | #38 | ETL-034: Data validation at each phase | #37 | Open |
| 7 | #5 | ETL-003: Replace .iterrows() with vectorized | #36, #35 | Open |

**Execution Order Rationale:**
1. **#31** - Must understand what tables are missing before verification
2. **#35** - Framework needed to validate everything else
3. **#36** - Tests needed before safe refactoring
4. **#13** - Goal counting is CRITICAL business rule
5. **#37** - Better errors help debugging
6. **#38** - Catch issues between phases
7. **#5** - Now safe to refactor with tests in place

### P1 Issues (High Priority)

| # | Title | Status |
|---|-------|--------|
| #4 | ETL-002: Profile ETL bottlenecks | Open |
| #6 | ETL-004: Create table verification script | Open |
| #7 | ETL-005: Verify all 139 tables have data | Open |
| #8 | ETL-006: Validate foreign key relationships | Open |
| #10 | ETL-008: Validate xG calculations | Open |
| #11 | ETL-009: Validate WAR/GAR calculations | Open |
| #12 | ETL-010: Validate Corsi/Fenwick calculations | Open |
| #25-29 | Vectorize specific functions | Open |
| #39 | ETL-029: Code walkthrough documentation | Open |
| #40 | ETL-036: Integration tests | Open |

### P2 Issues (Medium Priority)

| # | Title | Status |
|---|-------|--------|
| #9 | ETL-007: Identify unused tables | Open |
| #14 | ETL-012: Add verification to CI | Open |
| #16 | Remove dead code | Open |
| #30 | Remove data_loader.py wrapper | Open |
| #32-34 | Type hints, split functions, warnings | Open |

### Debug Infrastructure (Enhancement)

| # | Title | Status |
|---|-------|--------|
| #17-23 | ETL Debug: Docker PostgreSQL infrastructure | Open |

---

### Detailed Issue Descriptions

#### ETL-001: Modularize base_etl.py ✅ CLOSED (#3)
...
- **Status:** ✅ CLOSED - base_etl.py now ~1,065 lines + etl_phases/ modules (~4,700 lines)
- **Acceptance Criteria:**
  - [x] Create `src/core/etl_phases/` directory
  - [x] Extract derived columns → `derived_columns.py`
  - [x] Extract validation → `validation.py`
  - [x] Extract event enhancers → `event_enhancers.py`
  - [x] Extract shift enhancers → `shift_enhancers.py`
  - [x] Extract derived event tables → `derived_event_tables.py`
  - [x] Extract reference tables → `reference_tables.py`
  - [x] `base_etl.py` reduced to orchestration only (~1,065 lines)
  - [x] All tables still generated
  - [x] All tests pass
