# Complete PostgreSQL Schema Diagram - All 45 Tables
...
┌─────────────────────────────────────────────────────────────────────────────┐
│                              SOURCE FILES                                    │
│   BLB_Tables.xlsx (14 sheets) │ dim_dates.csv │ {game_id}_tracking.xlsx     │
└───────────────────────────────────────┬─────────────────────────────────────┘
                                        │
                                        ▼
┌─────────────────────────────────────────────────────────────────────────────┐
...
│  ┌──────────────────────────────────────────────────────────────────────┐   │
│  │ BLB Dimensions:                                                       │   │
│  │   stg_dim_player (335)      stg_dim_team (26)       stg_dim_league (2)│   │
│  │   stg_dim_schedule (552)    stg_dim_season (9)      stg_dim_dates (4747)│  │
│  │   stg_dim_seconds (4800)    stg_dim_randomnames (486)                 │   │
│  │   stg_dim_playerurlref (543) stg_dim_rinkboxcoord (50)               │   │
│  │   stg_dim_rinkcoordzones (297)                                        │   │
│  ├──────────────────────────────────────────────────────────────────────┤   │
│  │ BLB Facts:                                                            │   │
│  │   stg_fact_gameroster (14239)  stg_fact_draft (160)                   │   │
│  │   stg_fact_leadership (28)     stg_fact_registration (191)           │   │
│  │   stg_fact_playergames (3010)                                         │   │
│  ├──────────────────────────────────────────────────────────────────────┤   │
│  │ Game Tracking (per game):                                             │   │
│  │   stg_events_{game_id}    stg_shifts_{game_id}                        │   │
│  └──────────────────────────────────────────────────────────────────────┘   │
└───────────────────────────────────────┬─────────────────────────────────────┘
                                        │ SQL Transformations
                                        ▼
...
│  ┌──────────────────────────────────────────────────────────────────────┐   │
│  │ Dimensions:                                                           │   │
│  │   int_dim_player (335)     int_dim_team (26)     int_dim_schedule (552)│  │
│  │   int_dim_dates (4747)     int_dim_seconds (4800)                     │   │
│  ├──────────────────────────────────────────────────────────────────────┤   │
│  │ Facts:                                                                │   │
│  │   int_fact_gameroster (13960)                                         │   │
│  ├──────────────────────────────────────────────────────────────────────┤   │
│  │ Game Tracking (per game):                                             │   │
│  │   int_events_{game_id}       int_event_players_{game_id}              │   │
│  │   int_shifts_{game_id}       int_game_players_{game_id}               │   │
│  └──────────────────────────────────────────────────────────────────────┘   │
└───────────────────────────────────────┬─────────────────────────────────────┘
                                        │ Publish + Aggregate
                                        ▼
...
│  ┌──────────────────────────────────────────────────────────────────────┐   │
│  │ Dimension Tables (12):                                                │   │
│  │   dim_player (335)        dim_team (26)          dim_schedule (552)   │   │
│  │   dim_dates (4747)        dim_seconds (4800)     dim_period (5)       │   │
│  │   dim_event_type (7)      dim_strength (9)       dim_position (8)     │   │
│  │   dim_skill_tier (5)      dim_venue (2)          dim_zone (3)         │   │
│  ├──────────────────────────────────────────────────────────────────────┤   │
│  │ Fact Tables (3):                                                      │   │
│  │   fact_gameroster (13960)  fact_box_score (27+)   fact_events (1594+) │   │
│  └──────────────────────────────────────────────────────────────────────┘   │
└───────────────────────────────────────┬─────────────────────────────────────┘
                                        │ Export
                                        ▼
...
┌─────────────────────────────────────────────────────────────────────────────┐
│                          METADATA TABLES (2)                                 │
│   _blb_load_metadata (1)       _game_load_metadata (1)                       │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
#### stg_dim_league (2 rows)
| # | Column | Data Type |
|---|--------|-----------|
| 0 | league_id | TEXT |
| 1 | league | TEXT |

#### stg_dim_player (335 rows)
| # | Column | Data Type |
|---|--------|-----------|
...
#### stg_dim_rinkcoordzones (297 rows)
| # | Column | Data Type |
|---|--------|-----------|
| 0 | box_id | TEXT |
| 1 | box_id_rev | TEXT |
...
#### stg_events_{game_id} (~3,596 rows per game)
| # | Column | Data Type |
|---|--------|-----------|
...
#### stg_fact_gameroster (14,239 rows)
| # | Column | Data Type |
|---|--------|-----------|
| 0 | index | FLOAT |
| 1 | game_id | BIGINT |
...
#### stg_fact_playergames (3,010 rows)
| # | Column | Data Type |
|---|--------|-----------|
| 0 | ID | TEXT |
| 1 | Date | TEXT |
...
#### stg_fact_registration (191 rows)
| # | Column | Data Type |
|---|--------|-----------|
| 0 | player_full_name | TEXT |
| 1 | player_id | TEXT |
...
#### stg_shifts_{game_id} (~98 rows per game)
| # | Column | Data Type |
|---|--------|-----------|
...
#### int_event_players_{game_id} (~3,139 rows per game)
| # | Column | Data Type |
|---|--------|-----------|
| 0 | event_index | REAL |
| 1 | event_key | TEXT |
...
#### int_events_{game_id} (~1,594 rows per game)
| # | Column | Data Type |
|---|--------|-----------|
| 0 | event_index | REAL |
| 1 | event_key | TEXT |
...
#### int_fact_gameroster (13,960 rows)
| # | Column | Data Type |
|---|--------|-----------|
| 0 | game_id | INT |
| 1 | player_id | TEXT |
...
#### int_game_players_{game_id} (~27 rows per game)
| # | Column | Data Type |
|---|--------|-----------|
...
#### int_shifts_{game_id} (~98 rows per game)
| # | Column | Data Type |
|---|--------|-----------|
| 0 | shift_index | INT |
| 1 | shift_key | TEXT |
...
#### fact_box_score (27+ rows, grows with games)
| # | Column | Data Type |
|---|--------|-----------|
...
#### fact_events (1,594+ rows, grows with games)
| # | Column | Data Type |
|---|--------|-----------|
| 0 | event_index | FLOAT |
| 1 | event_key | TEXT |
