...
- 283 shifts don't have player attribution in fact_shift_players
...
@pytest.fixture(scope="module")
def fact_shift_players():
    return pd.read_csv(DATA_DIR / "fact_shift_players.csv")
...
class TestReferentialIntegrity:
    ...
    def test_all_shifts_player_reference_valid_players(self, fact_shift_players, dim_player):
        """Shift-player mappings must reference valid players"""
        shift_players = set(fact_shift_players['player_id'].dropna().unique())
        dim_players = set(dim_player['player_id'].unique())
...
class TestCrossTableConsistency:
    ...
    def test_shift_count_matches_shifts_player(self, fact_shifts, fact_shift_players):
        """Shift player coverage (known: 283 shifts without player attribution)"""
        if 'shift_key' in fact_shifts.columns and 'shift_key' in fact_shift_players.columns:
            shift_keys = set(fact_shifts['shift_key'].unique())
            player_shift_keys = set(fact_shift_players['shift_key'].unique())
...
class TestDeploymentSimulation:
    ...
    def test_all_core_tables_present(self):
        ...
        core_tables = [
            ...
            'fact_events_player.csv', 'fact_shift_players.csv',
