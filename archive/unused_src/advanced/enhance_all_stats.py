...
def main():
    ...
    events_player_df = pd.read_csv(OUTPUT_DIR / 'fact_event_players.csv')
    events_df = pd.read_csv(OUTPUT_DIR / 'fact_events.csv')
    game_player_stats_df = pd.read_csv(OUTPUT_DIR / 'fact_player_game_stats.csv')
    shifts_player_df = pd.read_csv(OUTPUT_DIR / 'fact_shift_players.csv')
    ...
    print(f"  - fact_event_players: {len(events_player_df)} rows")
    ...
    save_output_table(game_player_stats_df, 'fact_player_game_stats', OUTPUT_DIR)
    ...
    save_output_table(goalie_df, 'fact_goalie_game_stats', OUTPUT_DIR)
    ...
    if len(period_stats_df) > 0:
        save_output_table(period_stats_df, 'fact_player_period_stats', OUTPUT_DIR)
